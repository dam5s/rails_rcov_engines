...
if ARGV.grep(/--run-rake-task=/).empty?
  ...
  def new_rcov_task(test_name)
    plugin = ENV['PLUGIN'].to_s
    testing_plugin = test_name.include?('test:plugins') and plugin != ''
    output_dir = (testing_plugin ? "./coverage/#{plugin}" : "./coverage/#{test_name.gsub('test:','')}")
  ...
  module RcovTestSettings
    class << self
      attr_accessor :output_dir, :options, :untested_plugins
      def to_params
        "-o \"#{@output_dir}\" -T --exclude-only \"/Library*,rubygems/*,rcov*,config/*,environment/*,test/*,#{@untested_plugins.map{|p| "plugins/#{p}/*"}.join(',')}\" #{@options}"
      end
        ...
        Find.find('vendor/plugins/') do |path|
          if FileTest.directory?(path)
            if File.basename(path)[0] == ?.
              ...
              unless in_plugins_dir == true
                list << path.split('/').last
                # not recursively visiting children of children
                Find.prune
              else
                # first path visited is /vendor/plugins/ path
                in_plugins_dir = false
              end
            end
          end
        end
